...
<body>
	...
	<div>
		<a href="bookingListSelect?email=${bo.member_email}" >${bo.member_email } 뒤로가기</a>
	</div>
	<div>
		<form action="payment/payScreen" method = "get">
			<c:if test="${bo.bo_status eq '결제전' }">
				<div>
					<input type="submit" name = "booking_bo_num" value = "${bo.bo_num}">		
				</div>
				<div>
					<a href="bookingCancleUpdate?bo_num=${bo.bo_num}">${bo.bo_num }의 예약취소하기</a>
				</div>
			</c:if>
		</form>
		<form action="payment/payDetails" method = "get">
			<c:if test="${bo.bo_status eq '결제확인요청' }">
