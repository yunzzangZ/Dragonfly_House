...
public class FacilityController {
    ...
    // 전체 외부시설 조회
    @GetMapping(value = "business/facility/fac_listall")
    public ModelAndView fac_listall() {
        List<FacilityVO> list = service.fac_listAll();
        ModelAndView mav = new ModelAndView();
        mav.addObject("list", list);
        mav.setViewName("business/facility/fac_listall");
        return mav;
    }
    
    // 상세 조회
    @GetMapping(value = "business/facility/fac_selectOne")
    public ModelAndView selectOne(@RequestParam("fac_id") int fac_id) {
        ...
        mav.setViewName("business/facility/fac_selectOne");
        return mav;
    }
    
    // 추가 페이지 이동
    @GetMapping(value = "business/facility/fac_insert")
    public String insertForm(Model model) {
        model.addAttribute("facility", new FacilityVO()); // attribute 이름 수정
        return "business/facility/fac_insert";     
    }
    
    // 추가
    @PostMapping(value = "business/facility/fac_insert")
    @Transactional
    public String insert(FacilityVO fac_insert, RedirectAttributes rttr) {
        ...
        return "redirect:/business/facility/fac_listall";     
    }
    
    // 수정 이동
    @GetMapping(value = "business/facility/fac_update")
    public ModelAndView update(@RequestParam("fac_id") int fac_id) {
        ModelAndView mav = selectOne(fac_id);
        mav.setViewName("business/facility/fac_update");
        return mav;
    }
    
    // 수정
    @PostMapping(value = "business/facility/fac_update") // 수정된 부분
    @Transactional
    ...
    // 삭제
    @GetMapping(value = "business/facility/fac_delete") // 수정된 부분
    @Transactional
